# -*- coding: utf-8 -*-
"""
Created on Sun Nov 17 19:22:35 2013
...
##        random.seed(1) # used for testing
    ...
    while not breakpoint:
        if(len(ylist[ylist==-1]) == 0 or len(ylist[ylist==1]) == 0):
            for i in range (0,numberOfPoints - 1):
        ##        random.seed(1) # used for testing
                x = random.uniform (-1, 1)
                y = random.uniform (-1, 1)
...
def findErrorProbability(x1,y1,x2,y2, weights, numberOfPointsToTest, clf, pclf):
    numberOfErrors = 0
    points = list()
    for i in range(0, numberOfPointsToTest-1):
        ...
        points.append([x,y,targetFunction(x1,y1,x2,y2,x,y)])
    #print("out",numberOfErrors/float(numberOfPointsToTest))
    sample = np.array(points)
    X = np.c_[sample[:,0], sample[:,1]]
    y = sample[:,2]
    #print("Score", 1-lf.score(X,y))
    return 1-clf.score(X,y), numberOfErrors/float(numberOfPointsToTest), 1-pclf.score(X,y)
...
def runSimulation(numberOfTrials, numberOfTestPoints, iterationLimit):
    ...
    for t in range(1,numberOfTrials+1):
        ...
        probability.append(error_probability[1])
        k.append(error_probability[0])
        if(error_probability[0] < error_probability[1]):
            p.append(1)
        else:
            p.append(0)

        if(error_probability[0] < error_probability[2]):
            myP.append(1)
        else:
            myP.append(0)

        if(error_probability[1] < error_probability[2]):
            pc.append(1)
        else:
            pc.append(0)
